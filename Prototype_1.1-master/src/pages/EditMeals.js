/*  Components: EditMeals

    Functions: 
    
    Author: Skylar Riopel

    description: Page for editing meal in cart

    Notes: Removes old meal after  and add new one to end     Basically identical to meals except will edit meal in cart
     */
...
function EditMeals() {
    ...
    function handleOrder(jsonMeal) {
        ...
        delete temp1[editId];
        temp = JSON.stringify(temp1).replace(",null","");
    ...
    function onValueChange(value, number, drink, name, mi) {
        ...
        SidesItemJsonText += ']';
        orderItemJsonText = '{ "name":"' + mi.name + '" , "description":"' + mi.description + '", "picture":"' + mi.picture + '" ,"drinks":' + DrinkItemJsonText + ', "sides":' + SidesItemJsonText + ', "price":' + Number(mi.price + mealCost).toFixed(2) + '  }';
    ...
    return (
        <div data-testid="meals" className='restaurant'>
            <br />
            {
                tempRestsItems.map((restaurantItem, index) => {//show meal info and radio buttons for meals and sides
                        ...
                        return (
                            <div key={index} className="centered">
                                <div className="container1">
                                    ...
                                    <form action='/cart'>
                                        {
                                            restaurantItem.meals.map((mealsItem, index) => {//counting size of array
                                                if (mealsItem.id == mealId) {
                                                    ...
                                                    for (var i = 0; i < mealsItem.amtOfDrinks; i++) {//adding decision to array if chosen
                                                        drinksCntArray[i] = 0;
                                        ...
                                        {
                                            drinksCntArray.map((x, xIndex) => {//show drink options
                                                if (xIndex == 0) {//if to print title
                                                    return (
                                                        <div key={xIndex}>
                                                            <h4>Drinks:</h4>
                                                            {
                                        ...
                                        {
                                            sidesCntArray.map((x, xIndex) => {// show side items
                                                if (xIndex == 0) {//if to print title
                                                    return (
                                                        <div key={xIndex}>
                                                            <h4>Sides:</h4>
                                        ...
                                        }
                                        {
                                            restaurantItem.meals.map((mealsItem) => {
                                                if (mealsItem.id == mealId) {
                                                    ...
                                                    var foo = (drinkDecisionArr.length == mealsItem.amtOfDrinks && sideDecisionArr.length == mealsItem.amtOfSides && fail == 0);//give option to add to cart when inputs filled
                                                        return (
                                                            <div key={index}>
